...
const AddOrMultiply = (equation) => {
  ...
  while (true) {
    ...
    const currentResult = equationStack.reduce((currentResult, operationAndVal) => {
...
const result = equations.reduce((sum, currentEquation) => {
    console.log('---')
    if (AddOrMultiply(currentEquation)) return (sum += currentEquation.result);
    console.log(sum);
    return sum;
  }, 0)
console.log(result);
